...
"""
    MultimodalDataset class takes a CSV file with columns 'file_path', 'annotation_path', 'value' and 'additional_info' as input.
"""
class MultimodalDataset(Dataset):
    def __init__(self, csv_path, transform=None, tokenizer=None, model=None, use_paligemma=False, processor=None):
        """
        Args:
            csv_path (str): Path to CSV file with columns 'file_path' and 'annotation_path'
    ...
    def __load__(self, tokenizer, model):
        ...
        for _, row in self.data:
            print(f"row ===>><<<=== {row['annotation_path']}")
        ...
        print("About to create interleaving prompts")
        print(f"use paligemma {self.use_paligemma}")
        for data in all:
